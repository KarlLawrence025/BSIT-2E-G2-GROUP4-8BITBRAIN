...
// ============================================================
// CSV IMPORT — replaces old JSON import handler
// ============================================================
...
document.getElementById('import-btn')?.addEventListener('click', async () => {
    ...
    if (ext !== 'csv') {
        showToast('Only .csv files are supported. See the column format below.', 'error');
        return;
    }
    ...
    // ── POST as multipart/form-data (required for file upload) ──
    const formData = new FormData();
    formData.append('csv_file', file);

    try {
        const res    = await fetch('api/import_quiz_csv.php', {
            method: 'POST',
        ...
        if (result.success) {
            showToast(
                `✅ Imported ${result.quizzes_imported} quiz(zes), ${result.questions_imported} questions!`,
                'success'
            );
            fileInput.value = '';
            await loadQuizzes();
...
async function loadFeedback() {
    ...
    if (!result.success) { list.innerHTML = `<p class="empty-state">Failed to load feedback.</p>`; return; }
...
function buildFeedbackCard(f) {
    ...
    <div class="fb-card ${f.status}" data-id="${f.id}">
      ...
      <div class="fb-card-meta">
        <span class="fb-meta-item"><span class="fb-meta-icon">👤</span><span class="fb-meta-val">${esc(f.user_name || 'Anonymous')}</span></span>
        ${f.quiz_title ? `<span class="fb-meta-sep">·</span><span class="fb-meta-item"><span class="fb-meta-icon">📝</span><span class="fb-meta-val">${esc(f.quiz_title)}</span></span>` : ''}
        ${f.quiz_category ? `<span class="fb-meta-sep">·</span><span class="fb-meta-item"><span class="fb-meta-icon">🗂️</span><span class="fb-meta-val">${esc(f.quiz_category)}</span></span>` : ''}
        ${mode ? `<span class="fb-meta-sep">·</span><span class="fb-meta-item"><span class="fb-meta-icon">🎮</span><span class="fb-meta-val">${esc(mode)}</span></span>` : ''}
      </div>
...
async function loadReferences() {
    ...
    if (!result.success) { tbody.innerHTML = `<tr><td colspan="7" class="empty-state">Error: ${result.message}</td></tr>`; return; }
    ...
    tbody.innerHTML = result.data.map(ref => {
        ...
        const refLink   = ref.reference_url ? `<a href="${esc(ref.reference_url)}" target="_blank" style="color:#38bdf8;word-break:break-all;font-size:11px;">🔗 ${truncate(ref.reference_url, 45)}</a>` : (ref.reference_text ? `<span style="font-size:11px;color:#e0e0e0;">${esc(truncate(ref.reference_text, 60))}</span>` : '<span style="opacity:.4">—</span>');
        ...
        return `
        <tr>
            <td><div style="font-weight:bold;color:#fff;font-size:12px;">${esc(ref.quiz_title||'—')}</div><div style="font-size:10px;opacity:.5;margin-top:2px;">ID #${ref.quiz_id}</div></td>
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Update the import section label to reflect CSV
    const importSection = document.querySelector('.import-section h3');
    if (importSection) importSection.textContent = '📂 Import Quizzes from CSV';

    // Update file input to accept only CSV
    const fileInput = document.getElementById('quiz-file');
    if (fileInput) fileInput.setAttribute('accept', '.csv');

    // Add CSV format hint below the file input
    const importControls = document.querySelector('.import-controls');
    if (importControls && !document.getElementById('csv-hint')) {
        const hint = document.createElement('div');
        hint.id = 'csv-hint';
        hint.style.cssText = 'margin-top:12px;font-size:11px;color:rgba(255,255,255,.45);line-height:1.7;';
        hint.innerHTML = `
            <strong style="color:#00d9ff;">Required column order:</strong><br>
            <code style="color:#e0e0e0;">title, category, difficulty, mode, question, optionA, optionB, optionC, optionD, correct</code><br>
            <span style="opacity:.7;">
                difficulty → <em>easy / medium / hard</em> &nbsp;|&nbsp;
                mode → <em>single_player / timed_quiz / ranked_quiz / memory_match / endless_quiz</em> &nbsp;|&nbsp;
                correct → <em>A / B / C / D</em>
            </span>`;
        importControls.after(hint);
    }
    ...
    .fb-status-pill { font-size:11px; font-weight:700; padding:4px 12px; border-radius:20px; font-family:'Courier New',monospace; letter-spacing:.3px; }
    ...
    .fb-card-date   { font-size:11px; color:rgba(255,255,255,.35); font-family:'Courier New',monospace; }
    ...
    .fb-action-btn.resolve { background:rgba(57,255,20,.1); border-color:#39ff14; color:#39ff14; }
    .fb-action-btn.resolve:hover { background:#39ff14; color:#1a0b2e; }
    .fb-action-btn.del { background:rgba(255,23,68,.1); border-color:#ff1744; color:#ff1744; }
    .fb-action-btn.del:hover { background:#ff1744; color:#fff; }
    `;
    document.head.appendChild(style);

    console.log('✨ Admin Dashboard ready — CSV import enabled');
});
