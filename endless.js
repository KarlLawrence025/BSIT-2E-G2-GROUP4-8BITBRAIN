...
// ── State ─────────────────────────────────────────────────────────────────────
let questions    = [];   // all questions fetched from DB
let qIndex       = 0;   // current position
let lives        = 3;
let score        = 0;
let streak       = 0;
let bestStreak   = 0;
...
function renderQuestion() {
    ...
    // If we've gone through all questions, loop (shuffle again)
...
function updateProgress() {
    ...
    // Fill bar based on accuracy when answered > 0
...
async function saveResult() {
    try {
        await fetch("api/save_quiz_result.php", {
            method: "POST",
...
function show(id) {
    ["elLoading","elGame","elResults"].forEach(k => {
        ...
        el.style.display = k === id
            ? (k === "elGame" ? "flex" : k === "elResults" ? "flex" : "flex")
            : "none";
    });
    // elGame uses flex-direction:column
